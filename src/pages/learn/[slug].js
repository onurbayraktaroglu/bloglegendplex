...
export default function LearnDetailPage({ learn }) {
    const [listDetail, setListDetail] = useState(LearnList.List);
    const [newList, setNewList] = useState(learn);
    ...
    <main className='main' style={{ marginTop: '72px' }}>
        <div className='container'>
            <div className='row'>
                ...
                <Button className="btn btn-light border mt-4" variant="primary" onClick={onBackHistory} style={{ borderRadius: '20px', width: '100px' }}>&#60; Back</Button>
                <div className='detail-breadcrumb'>
                    <Link href="/learn">Learn </Link>&nbsp;/ <span>&nbsp;{newList.url} / </span>
                </div>
...
export const getServerSideProps = async (context) => {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL}/data/learnData.json`).then((r) => r.json());
